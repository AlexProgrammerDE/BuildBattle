...
public class TeamArena extends SoloArena {
  ...
  public void distributePlots() {
    ...
    for (Plot plot : getPlotManager().getPlots()) {
      ...
      pairs
          .get(i)
          .forEach(player -> getPlugin().getUserManager().getUser(player).setCurrentPlot(plot));
      i++;
    }
    /*if (!players.isEmpty()) {
  ...
  public String formatWinners(Plot plot, String string) {
    ...
    if (plot.getOwners().size() == 1) {
      ...
      str =
          str.replaceAll(
              "(?i)%player%",
              plot.getOwners().get(0).getName() + " & " + plot.getOwners().get(1).getName());
    } else {
  ...
  public boolean enoughPlayersToContinue() {
    if (getPlayers().size() >= 2) {
      ...
      return !getPlotManager()
          .getPlot(getPlayers().get(0))
          .getOwners()
          .contains(getPlayers().get(1));
    } else {
      return false;
    }
  }
