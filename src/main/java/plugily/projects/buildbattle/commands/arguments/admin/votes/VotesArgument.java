  ...
  public VotesArgument(ArgumentsRegistry registry) {
    registry.mapArgument(
        ...
        new LabeledCommandArgument(
            "votes",
            "buildbattle.admin.supervotes.manage",
            CommandArgument.ExecutorType.BOTH,
            new LabelData(
                "/bba votes &6<add/set> <amount> &c[player]",
                "/bba votes <action> <amount>",
          ...
          public void execute(CommandSender sender, String[] args) {
            ...
            if (args.length == 2) {
              sender.sendMessage(
                  registry.getPlugin().getChatManager().getPrefix()
                      + ChatColor.RED
                      + "Please type amount of super votes to add or set!");
              return;
            }
            if (!args[1].equalsIgnoreCase("add") && !args[1].equalsIgnoreCase("set")) {
              sender.sendMessage(
                  registry.getPlugin().getChatManager().getPrefix()
                      + ChatColor.RED
                      + "Wrong command usage, please use /bba votes <add|set> <amount> [player]!");
              return;
            }
            Player target;
            if (args.length == 3) {
              if (!(sender instanceof Player)) {
                sender.sendMessage(
                    registry.getPlugin().getChatManager().getPrefix()
                        + ChatColor.RED
                        + "Console can't recieve supervotes.");
                return;
              }
            ...
            if (target == null) {
              sender.sendMessage(
                  registry.getPlugin().getChatManager().colorMessage("Commands.Player-Not-Found"));
              return;
            }
            if (NumberUtils.isInteger(args[2])) {
                ...
                successMessage =
                    registry.getPlugin().getChatManager().getPrefix()
                        + registry
                            .getPlugin()
                            .getChatManager()
                            .colorMessage("Commands.Arguments.Super-Votes-Added");
                ...
                successMessage =
                    registry.getPlugin().getChatManager().getPrefix()
                        + registry
                            .getPlugin()
                            .getChatManager()
                            .colorMessage("Commands.Arguments.Super-Votes-Set");
                successMessage = StringUtils.replace(successMessage, "%amount%", args[2]);
              }
              // todo translatable?
              sender.sendMessage(
                  registry.getPlugin().getChatManager().getPrefix()
                      + registry
                          .getPlugin()
                          .getChatManager()
                          .colorRawMessage("&aSuper votes amount modified!"));
              ...
              sender.sendMessage(
                  registry
                      .getPlugin()
                      .getChatManager()
                      .colorRawMessage("&cArgument isn't a number!"));
            }
          }
        });
  }
