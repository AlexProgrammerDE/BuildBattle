    ...
    registry.mapArgument(
        ...
        new LabeledCommandArgument(
          ...
          public void execute(CommandSender sender, String[] args) {
              ...
              player.sendMessage(
                  registry.getPlugin().getChatManager().getPrefix()
                      + registry
                          .getPlugin()
                          .getChatManager()
                          .colorRawMessage("&cPlease type arena name!"));
              return;
            }
            ...
            CuboidSelector.Selection selection =
                registry.getPlugin().getCuboidSelector().getSelection(player);
            if (selection == null
                || selection.getFirstPos() == null
                || selection.getSecondPos() == null) {
              player.sendMessage(
                  registry
                      .getPlugin()
                      .getChatManager()
                      .colorRawMessage(
                          registry.getPlugin().getChatManager().getPrefix()
                              + "&cPlease select both corners before adding a plot!"));
              return;
            }
            ...
            LocationSerializer.saveLoc(
                registry.getPlugin(),
                config,
                "arenas",
                "instances." + arena.getID() + ".plots." + id + ".minpoint",
                selection.getFirstPos());
            LocationSerializer.saveLoc(
                registry.getPlugin(),
                config,
                "arenas",
                "instances." + arena.getID() + ".plots." + id + ".maxpoint",
                selection.getSecondPos());
            player.sendMessage(
                registry.getPlugin().getChatManager().getPrefix()
                    + registry
                        .getPlugin()
