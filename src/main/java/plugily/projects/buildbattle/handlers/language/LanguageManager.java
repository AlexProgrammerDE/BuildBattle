...
public class LanguageManager {
  ...
  private static void registerLocales() {
    Arrays.asList(
            new Locale(
                ...
                Arrays.asList("basque", "eu")),
            new Locale(
                "Belarusian",
                "Belarus",
                "be_BY",
                "POEditor contributors",
                Arrays.asList("belarusian", "belarus", "by", "be")),
            new Locale(
                "Chinese (Simplified)",
                "简体中文",
                "zh_CN",
            ...
            new Locale(
                ...
                "zh_HK",
                "POEditor contributors",
                Arrays.asList("中文(傳統)", "中國傳統", "chinese_traditional", "zh_hk")),
            new Locale(
                "Czech",
                "Český",
                "cs_CZ",
            ...
            new Locale(
                ...
                "et_EE",
                "POEditor contributors",
                Arrays.asList("estonian", "eesti", "et")),
            new Locale(
                "French",
                "Français",
                "fr_FR",
            ...
            new Locale(
                ...
                "ja_JP",
                "POEditor contributors",
                Arrays.asList("日本語", "japanese", "jp", "ja")),
            new Locale(
                "Korean",
                "한국의",
                "ko_KR",
            ...
            new Locale(
                ...
                "lt_LT",
                "POEditor contributors",
                Arrays.asList("lithuanian", "lietuvių", "lietuviu", "lt")),
            new Locale(
            ...
            new Locale(
                ...
                "pt_BR",
                "POEditor contributors",
                Arrays.asList("portuguese br", "português br", "português brasil", "pt_br")),
            new Locale(
                "Romanian",
                "Românesc",
                "ro_RO",
            ...
            new Locale(
                ...
                "ru_RU",
                "POEditor contributors",
                Arrays.asList("russian", "russkiy", "pусский", "ru")),
            new Locale(
                "Slovenian",
                "Slovenščina",
                "sl_SL",
                "POEditor contributors",
                Arrays.asList("slovenian", "slovenščina", "slovenscina", "sl")),
            new Locale(
                "Spanish",
                "Español",
                "es_ES",
  ...
  private static void loadProperties() {
    ...
    if (service == null) {
      Debugger.sendConsoleMsg(
          "&c[Build Battle] Locales cannot be downloaded because API website is unreachable, locales will be disabled.");
    ...
    if (service.isValidVersion()) {
      ...
      if (status == LocaleService.DownloadStatus.FAIL) {
        pluginLocale = LocaleRegistry.getByName("English");
        Debugger.sendConsoleMsg(
            "&c[BuildBattle] Locale service couldn't download latest locale for plugin! English locale will be used instead!");
        return;
      } else if (status == LocaleService.DownloadStatus.SUCCESS) {
        Debugger.sendConsoleMsg(
            "&c[BuildBattle] Downloaded locale " + pluginLocale.getPrefix() + " properly!");
      } else if (status == LocaleService.DownloadStatus.LATEST) {
        Debugger.sendConsoleMsg(
            "&c[BuildBattle] Locale " + pluginLocale.getPrefix() + " is latest! Awesome!");
      }
    } else {
      pluginLocale = LocaleRegistry.getByName("English");
      Debugger.sendConsoleMsg(
          "&c[BuildBattle] Your plugin version is too old to use latest locale! Please update plugin to access latest updates of locale!");
      return;
    }
    try {
      properties.load(
  ...
  private static void setupLocale() {
    ...
    /* is beta release */
    if (plugin.getDescription().getVersion().contains("locales")
        || plugin.getDescription().getVersion().contains("pre")) {
      Debugger.sendConsoleMsg(
          "&c[BuildBattle] Locales aren't supported in beta versions because they're lacking latest translations! Enabling English one...");
      pluginLocale = LocaleRegistry.getByName("English");
      return;
    }
    Debugger.sendConsoleMsg(
        "&a[BuildBattle] Loaded locale "
            + pluginLocale.getName()
            + " ("
            + pluginLocale.getOriginalName()
            + " ID: "
            + pluginLocale.getPrefix()
            + ") by "
            + pluginLocale.getAuthor());
    loadProperties();
  }
  ...
  private static List<String> getStrings(String path) {
    ...
    if (!language.isSet(path)) {
      ...
      if (!language.isSet(path)) {
        ...
        Debugger.sendConsoleMsg(
            "&c[BuildBattle] Please regenerate your language.yml file! If error still occurs report it to the developer on discord!");
    ...
    list =
        list.stream()
            .map(string -> plugin.getChatManager().colorRawMessage(string))
