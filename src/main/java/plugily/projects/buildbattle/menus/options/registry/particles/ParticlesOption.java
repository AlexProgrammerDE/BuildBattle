        ...
        new MenuOption(
            ...
            new ItemBuilder(XMaterial.DANDELION.parseItem())
                .name(
                    registry
                        .getPlugin()
                        .getChatManager()
                        .colorMessage("Menus.Option-Menu.Items.Particle.Item-Name"))
                .lore(
                    registry
                        .getPlugin()
                        .getChatManager()
                        .colorMessage("Menus.Option-Menu.Items.Particle.Item-Lore"))
                .build(),
            registry
                .getPlugin()
                .getChatManager()
          ...
          public void onTargetClick(InventoryClickEvent e) {
            ...
            if (e.getCurrentItem()
              ...
              e.setCancelled(false);
              e.getWhoClicked().closeInventory();
              ParticleRemoveMenu.openMenu(
                  (Player) e.getWhoClicked(),
                  arena.getPlotManager().getPlot((Player) e.getWhoClicked()));
              return;
            }
            for (ParticleItem particleItem :
                registry.getParticleRegistry().getRegisteredParticles()) {
              // Only check for the display name for items in gui, because some of item meta is
              // changed
              if (!e.getCurrentItem()
                  .getItemMeta()
                  .getDisplayName()
                  .contains(particleItem.getItemStack().getItemMeta().getDisplayName())) {
                continue;
              }
                    ...
                    .sendMessage(
                        registry.getPlugin().getChatManager().getPrefix()
                            + registry
                                .getPlugin()
                                .getChatManager()
                                .colorMessage("In-Game.No-Permission-For-Particle"));
                continue;
              }
              if (plot.getParticles().size()
                  >= registry.getPlugin().getConfig().getInt("Max-Amount-Particles", 25)) {
                e.getWhoClicked()
                    .sendMessage(
                        registry.getPlugin().getChatManager().getPrefix()
                            + registry
                                .getPlugin()
                                .getChatManager()
                                .colorMessage("In-Game.Max-Particles-Limit-Reached"));
                return;
              }
              ...
              registry
                  .getPlugin()
                  .getUserManager()
                  .getUser((Player) e.getWhoClicked())
                  ...
                  .sendMessage(
                      registry.getPlugin().getChatManager().getPrefix()
                          + registry
                              .getPlugin()
                              .getChatManager()
                              .colorMessage("In-Game.Particle-Added"));
            }
          }
        });
  }
